// ported from desktop/desktop app/src/lib/editors/win32.ts
// see src/editors/upstream.json and UPDATING.md to re-sync
...
let jetBrainsToolboxEditors:
  | Promise<ReadonlyArray<WindowsExternalEditor>>
  | undefined

// memoize-one upstream, but the call takes no arguments so a cached
// promise does the same job without the dependency
const getJetBrainsToolboxEditors = () =>
  (jetBrainsToolboxEditors ??= (async () => {
...
export async function getAvailableEditors(): Promise<
  ReadonlyArray<IFoundEditor>
> {
  const results: Array<IFoundEditor> = []
  const candidates = [
    ...editors,
    ...(await getJetBrainsToolboxEditors().catch(() => [])),
  ]
